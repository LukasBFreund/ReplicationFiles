...
parameters 
  ...
  // alp            ${\alpha}$              (long_name='capital share (exluding profits)')
   ...
   //phi_p          ${\phi_p}$              (long_name='Rotemberg price adj. costs')
   //phi_w          ${\phi_w}$              (long_name='Rotemberg wage adj. costs')
...
util       = 0.4951; %util=0.01 = no utilization // the highe util the higher the utilization in the model (usually calibrated in between 0.5 and 1)
...
%%%%%%%AVERAGE ESTIMATION%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% rhoG       = (0.9222 + 0.9207 )/2;  %autoregressive param G  shock (Government Spending)
% phiX       = (2.4605 + 1.8149 )/2; %Investment adjustment costs
% calvo_w    = (0.7085 + 0.7773  )/2; %1-1/s_wages_duration;
% calvo      = (0.8635 + 0.8556 )/2; %1-1/s_prices_duration;
% theta_pie  = (1.7268 + 1.7470 )/2; %Taylor rule elasticity of interest rate to inflation
% rho_r      = (0.5388 + 0.6038 )/2; %Taylor rule interest rate smoothing
% theta_y    = (0.1339 + 0.1227 )/2; %Taylor rule elasticity of interest rate to output
...
%#psi=(zzeta-1)/phi_p; %slope phillips curve
%#calvo_w=1-1/s_wages_duration;
%#phi_w=calvo_w*(zzeta_w-1)/((1-calvo_w)*(1-betta*calvo_w)); % implied Rotemberg parameter (exploit first-order equivalence) and LSss=1!
%#LSss=Wss*Hss/Yss;
%#psi_w=(zzeta_w-1)*LSss/phi_w;  %slope wage phillips curve
...
PIE=betta*PIE(+1)+(1-betta*calvo)*(1-calvo)/(calvo)*(MC+MS);

[name='Linear Wage Phillips Curve '] 
W=betta/(1+betta)*W(+1)+1/(1+betta)*W(-1)+betta/(1+betta)*PIE(+1)-1/(1+betta)*PIE+(1-betta*calvo_w)*(1-calvo_w)/((1+betta)*calvo_w)*(MRS-W+WMS);
