...
s_prices_duration s_wages_duration calvo calvo_w sigmaG 
...
@#if model_version == 0
  ...
  */
  ...
  clear G_resp_epsG Y_resp_epsG LS_resp_epsG C_resp_epsG I_resp_epsG PIE_resp_epsG G_respSE_epsG Y_respSE_epsG LS_respSE_epsG C_respSE_epsG I_respSE_epsG PIE_respSE_epsG Var_IRF
...
%% Select IRFs to match
IRFFF=Y([1:6],:)';  
IRFFFSE=s_Y([1:6],:)';

% Priors 
estimated_params;   
rhoG,.85,,,beta_pdf,0.7,0.15; 
kappa,,,,beta_pdf,0.1,0.02; 
kappaw,,,,beta_pdf,0.1,0.02; 
lambda,0.7,0,1,normal_pdf,0.5,0.15;

phiX,,,,gamma_pdf,4,2; 
psiH,,,,normal_pdf,0.2265,0.1; 
rho_r,,,,beta_pdf,0.7,0.15; 
theta_pie ,,,,gamma_pdf,1.7,0.15;
theta_y  ,,,,gamma_pdf,0.1,0.05;
sigmaG,,,,gamma_pdf,1,.05;     
end;      
...
% Settings
mod_var_list = { 'G' 'Y' 'LS' 'C' 'I' 'PIE' };                                                % List variables to match
mod_shock_list = {'epsG'};                                          % Shocks conditional on which to match
options_.irfs_match_estimation = 1;                                        % IRF Matching
horizon_est  = 15;                                                         % Horizon to match

%IRFs for matching
G_resp_epsG=IRFFF(1:horizon_est,1);
Y_resp_epsG=IRFFF(1:horizon_est,2);
LS_resp_epsG=IRFFF(1:horizon_est,3);
C_resp_epsG=IRFFF(1:horizon_est,4);
I_resp_epsG=IRFFF(1:horizon_est,5);
PIE_resp_epsG=IRFFF(1:horizon_est,6);


G_respSE_epsG=IRFFFSE(1:horizon_est,1);
Y_respSE_epsG=IRFFFSE(1:horizon_est,2);
LS_respSE_epsG=IRFFFSE(1:horizon_est,3);
C_respSE_epsG=IRFFFSE(1:horizon_est,4);
I_respSE_epsG=IRFFFSE(1:horizon_est,5);
PIE_respSE_epsG=IRFFFSE(1:horizon_est,6);

% Stack by (horizon * var * shock * obs)
%Var_IRF(:,:,1,:) =[IRFFFSE(1:horizon_est,1) IRFFFSE(1:horizon_est,2) IRFFFSE(1:horizon_est,3) IRFFFSE(1:horizon_est,4) IRFFFSE(1:horizon_est,5)] ;

% Settings
StdNorm          = [G_respSE_epsG Y_respSE_epsG LS_respSE_epsG C_respSE_epsG I_respSE_epsG PIE_respSE_epsG];%std(Var_IRF,0,4);
Var_IRF          = [G_resp_epsG Y_resp_epsG LS_resp_epsG C_resp_epsG I_resp_epsG PIE_resp_epsG];% mean(Var_IRF,4);
Var_IRF          = Var_IRF(:);
StdNorm          = StdNorm(:);
VarNorm          = StdNorm.^2;
...
// Estimation
%--------------------------------------------------------------------------
estimation(datafile='fsdat.mat',mh_replic=500000,mh_nblocks=3,mh_jscale=0.7,mode_compute = 0,mode_check,plot_priors = 1,mode_file=tank_cw_mode);%,load_mh_file);
...
FigSize
subplot(2,3,1)
HX = PlotSwathe(Y_resp_epsG,Y_respSE_epsG); hold on; h = plot([oo_.irfs.Y_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('Y to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
subplot(2,3,2)
HX = PlotSwathe(G_resp_epsG,G_respSE_epsG); hold on; h = plot([oo_.irfs.G_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('G to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
subplot(2,3,3)
HX = PlotSwathe(LS_resp_epsG,LS_respSE_epsG); hold on; h = plot([oo_.irfs.LS_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('LS to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
subplot(2,3,4)
HX = PlotSwathe(C_resp_epsG,C_respSE_epsG); hold on; h = plot([oo_.irfs.C_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('C to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
subplot(2,3,5)
HX = PlotSwathe(I_resp_epsG,I_respSE_epsG); hold on; h = plot([oo_.irfs.I_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('I to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
subplot(2,3,6)
HX = PlotSwathe(PIE_resp_epsG,PIE_respSE_epsG); hold on; h = plot([oo_.irfs.PIE_epsG(1:horizon_est)'],'--','LineWidth',1.5,'Color',cmap(2)); 
title('PIE to epsG'); legend([HX.patch h],{'VAR';'Estimated DSGE'})
